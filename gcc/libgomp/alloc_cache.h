...
alloc_cache_try_find (struct alloc_cache *cache, size_t desired_size)
{
  for (struct alloc_cache_node *node =
	...
	continue;
      ...
      if (pthread_mutex_trylock (&node->lock) != 0)
	continue;
#else
      {
	int64_t timeout = 0;
	if (_MCF_mutex_lock (&node->lock, &timeout) != 0)
	  continue;
      }
#endif
			    ...
			    size_t size)
{
  ...
  *new_node = (struct alloc_cache_node) {
    .lock = GOMP_MUTEX_INIT_0,
    .allocation = allocation,
    .size = size,
    .next = NULL
  };
  gomp_mutex_lock (&new_node->lock);
